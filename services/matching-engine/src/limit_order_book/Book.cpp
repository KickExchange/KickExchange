...
void Book::marketOrderHelper(int orderId, bool buyOrSell, int shares)
{
    ...
    while (bookEdge != nullptr && bookEdge->getHeadOrder()->getShares() <= shares)
    {
        ...
        int restingOrderId = headOrder->getOrderId();
        int filledShares = headOrder->getShares();
        int price = headOrder->getLimit();
        shares -= filledShares;
        headOrder->execute();
        if (bookEdge->getSize() == 0)
        {
        ...
        deleteFromOrderMap(restingOrderId);
        // limitOrders.erase(headOrder);
        delete headOrder;
        executedOrdersCount += 1;
        if (onTrade) onTrade(TradeEvent{restingOrderId, orderId, filledShares, price, 0});
    }
    if (bookEdge != nullptr && shares != 0)
    {
        ...
        executedOrdersCount += 1;
        if (onTrade) onTrade(TradeEvent{restingOrderId, orderId, shares, price, headOrder->getShares()});
    }
}
