...
// One match between an incoming order and a resting order. incomingOrderId
// is 0 for the rare synthetic stop->market conversion path where the
// original order id isn't tracked (pre-existing engine behavior).
struct TradeEvent {
    int restingOrderId;
    int incomingOrderId;
    int shares;
    int price;
    int restingRemainingShares;  // 0 if the resting order was fully filled and removed
};
...
public:
    ...
    // Invoked once per match (fill or partial fill) during order processing.
    // Unset by default - callers that don't care about trade events pay
    // nothing (checked before each call, no-op std::function is cheap but
    // this skips even that).
